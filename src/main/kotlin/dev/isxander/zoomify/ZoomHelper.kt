...
class ZoomHelper(private val starting: Double = 1.0) {
    ...
    private fun tickScroll(scrollTiers: Int, lastFrameDuration: Double) {
        ...
        val smoothness = lerp(ZoomifySettings.scrollZoomSmoothness / 100.0, 1.0, 0.1)
    ...
    private fun getInitialZoomMultiplier(tickDelta: Float): Double {
        return lerp(
            activeTransition.apply(lerp(tickDelta.toDouble(), prevInitialInterpolation, initialInterpolation)),
            1 / starting,
    ...
    private fun getScrollZoomDivisor(tickDelta: Float): Double {
        return lerp(
            lerp(tickDelta.toDouble(), prevScrollInterpolation, scrollInterpolation),
            0.0,
